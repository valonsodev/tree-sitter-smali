...
/**
 * Matches zero or more comma-separated occurrences of `rule`.
 * @param {Rule} rule
 * @return {ChoiceRule}
 */
...
module.exports = grammar({
  ...
  externals: $ => [
    $.L,
    $._class_ident,
    $._error_sentinel, // unused token for error recovery detection (see scanner.c)
  ],
  ...
  rules: {
    ...
    annotation_key: _ => new RustRegex('[\\p{L}_$][\\p{L}\\p{N}_\\-$]*'),
    ...
    class_identifier: $ => seq(
      ...
      repeat(seq('/', alias($._class_ident, $.identifier))),
      ';',
    ),
    ...
    array_type: $ => seq('[', $.type),
    // Both precedence levels are needed: the unprecedented form handles primitives
    // appearing as expression values, while the precedented form resolves the ambiguity
    // in method signatures where single-letter primitives (I, V, etc.) compete with identifiers.
